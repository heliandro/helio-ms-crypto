...
describe('KeyPair', () => {
    ...
    test('Deve lançar um erro ao instanciar a classe com uma chave publica inválida', () => {
        expect(() => new KeyPair('chave publica inválida', '')).toThrow(new Error(`A(s) chave(s) de criptografia pem são inválidas`))
    })

    test('Deve lançar um erro ao instanciar a classe com uma chave privada inválida', () => {
        expect(() => new KeyPair('', 'chave privada inválida')).toThrow(new Error(`A(s) chave(s) de criptografia pem são inválidas`))
    })
})