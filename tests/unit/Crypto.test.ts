...
describe('Crypto', () => {
    ...
    describe('Cenários de Erro', () => {
        test('Deve lançar um erro ao gerar o par de chaves de criptografia', async () => {
            // Given
            const stub = sinon
                .stub(NodeCrypto, 'generateKeyPairSync')
                .throwsException('Internal error');
            // When - Then
            await expect(() => Crypto.create()).rejects.toThrow(
                new Error('Falha ao gerar o par de chaves de criptografia.')
            );
            stub.restore();
        });
        ...
        test('Deve lançar um erro ao instanciar com um par de chaves de criptografia inválido', async () => {
            ...
            // When - Then
            expect(() => new Crypto(publicKey, privateKey)).toThrow(
        ...
        test('Deve lançar um erro ao desencriptar um dado inválido', async () => {
            ...
            const crypto = await Crypto.create();
            // When
            expect(() => crypto.decrypt(invalidDataFormatInBase64)).toThrow(
                new Error('Falha ao desencriptar os dados.')
            );
        });
    });
});
